//Retrieved from stanford notebook


//Initialize with the array.
// Array must be indexed from 0 to list.length-1;
// Segment tree has two main methods.
// 1. update(begin,end,val) increments the range begin,end (inclusive) with value v.  (O (log n))
// 2. query(begin,end) returns the sum of the elements between begin and end(inclusive). For example, query(i,i) returns the i-th element. (O(log n))
